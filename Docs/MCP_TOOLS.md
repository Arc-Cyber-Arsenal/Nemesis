...
Nemesis exposes tools through the Model Context Protocol (MCP). These tools give AI assistants the ability to:
...
Nemesis ships two transports. stdio is the default and spawns the MCP server as a local subprocess. HTTP (Streamable HTTP, spec 2025-11-25) is optional and lets remote clients connect with a Bearer API key.

### stdio (default)

```bash
python3 nemesis.py --assessment "my-target"
```

Generates `.nemesis/mcp-config.json` pointing at the backend venv's Python. No network exposure — the AI client and MCP server share stdin/stdout.

### HTTP (optional, remote-capable)

1. Open Nemesis → Settings → **MCP Access**.
...
The endpoint is reachable at three URLs depending on how Nemesis was started:
...
```bash
# Local
claude mcp add --transport http nemesis http://localhost:8000/mcp \
  --header "Authorization: Bearer nemesis_sk_..."

# LAN — use the LAN IP shown by ./start.sh --lan
claude mcp add --transport http nemesis https://192.168.1.42/mcp \
  --header "Authorization: Bearer nemesis_sk_..."

# Public domain
claude mcp add --transport http nemesis https://nemesis.example.com/mcp \
  --header "Authorization: Bearer nemesis_sk_..."
```
...
```json
{
  "mcpServers": {
    "nemesis": {
      "url": "https://nemesis.example.com/mcp",
      "headers": { "Authorization": "Bearer nemesis_sk_..." }
    }
  }
}
```
...
```bash
python3 nemesis.py --http http://localhost:8000/mcp --mcp-api-key nemesis_sk_...
```
