...
class NotificationService:
    ...
    async def test_channel(self, channel: str) -> tuple[bool, str]:
        ...
        test_message = "Nemesis Test Notification - Your notification channel is configured correctly!"
    ...
    def build_assessment_report(
        ...
        lines = []
        lines.append(f"{'='*40}")
        lines.append(f"Nemesis Assessment Report")
        lines.append(f"{'='*40}")
        ...
        lines.append(f"{'='*40}")
        lines.append("Generated by Nemesis")
        return "\n".join(lines)
    ...
    async def send_report(
        ...
        filename = f"Nemesis_Report_{safe_name}.pdf"

        # Build a short caption
        caption = f"Nemesis Report: {assessment.name}" if assessment else "Nemesis Report"
        if custom_message:
            caption += f"\n{custom_message}"
    ...
    async def _send_slack_file(self, cfg: Dict[str, Any], file_bytes: bytes, filename: str, caption: str):
        ...
        async with httpx.AsyncClient(timeout=10) as client:
            resp = await client.post(webhook_url, json={
                "text": f"{caption}\n_PDF report generated. Use the Nemesis web interface to download it._"
            })
            if resp.status_code != 200:
    ...
    async def _send_email_file(self, cfg: Dict[str, Any], file_bytes: bytes, filename: str, caption: str):
        ...
        msg["Subject"] = f"Nemesis Report - {filename}"
    ...
    async def _send_email(self, cfg: Dict[str, Any], message: str):
        ...
        subject = cfg.get("subject", "Nemesis Security Alert")
