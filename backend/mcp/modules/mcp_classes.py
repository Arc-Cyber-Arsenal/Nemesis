"""
MCP Classes - Nemesis MCP Service with structured logging
"""
import asyncio
import time
import json
...
class AidaMCPService:
    ...
    def _read_session_file() -> Optional[str]:
        """Read cached token from .nemesis/api-key or .nemesis/session.

        Checks api-key first (long-lived, 1 year) then falls back to
        the short-lived session token written by nemesis.py.

        Path resolution: this file is at Nemesis/backend/mcp/modules/mcp_classes.py
        so parents[3] = Nemesis root.
        """
        nemesis_root = Path(__file__).resolve().parents[3]
        for filename in ("api-key", "session"):
            try:
                path = nemesis_root / ".nemesis" / filename
                if path.exists():
    ...
    def __init__(self, backend_url: str = None):
        ...
        self.token: Optional[str] = os.getenv("Nemesis_TOKEN") or self._read_session_file()
        ...
        self.claude_container_name: str = os.getenv("DEFAULT_CONTAINER_NAME", "nemesis-pentest")
                ...
                for line in result["stdout"].split('\n'):
                    if line.strip():
                        try:
                            ...
                            # Include Exegol containers and Nemesis built-in containers
                            is_exegol = ("exegol" in image.lower() or
                                         "nwodtuhs/exegol" in image.lower())
                            is_nemesis = ("nemesis" in image.lower() and "pentest" in image.lower())
                            is_default = (name == self.claude_container_name)

                            if is_exegol or is_nemesis or is_default:
                                containers.append({
                                    "name": name,
                                    "image": image,
